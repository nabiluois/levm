// ============================================
// SYSTEME EN LIGNE - V34 (PASTILLES & FICHE TECHNIQUE)
// ============================================
...
window.generateResurrectionGrid = function(mode = 'single') {
    ...
    if (mode === 'multi') {
        ...
        dashboard.innerHTML = `
            ...
            <button class="btn-validate btn-validate-small" style="background:#2ecc71;" onclick="window.validateDistribution()">✅ VALIDER (<span id="pop-total">0</span>)</button>
        `;
            ...
            rolesInCat.forEach(role => {
                ...
                if (mode === 'multi') {
                    ...
                    if (count > 0) {
                        ...
                        if (badgeCards.some(id => role.id.includes(id))) { div.innerHTML += `<div class="qty-badge">x${count}</div>`; }
                    }
                }
                ...
                div.onclick = function() { if (mode === 'multi') { handleMultiSelection(role.id, div); } else { window.assignRoleToPlayer(role.id); } };
...
function handleMultiSelection(roleId, divElement) {
    ...
    if (newCount > 0) {
        ...
        if (badgeCards.some(id => roleId === id)) { 
            ...
            if (!badge) { badge = document.createElement('div'); badge.className = 'qty-badge'; divElement.appendChild(badge); }
...
window.validateDistribution = function() { window.closeModal('modal-role-selector'); generateRoleChecklist(); };
window.openDistributionSelector = function() { window.generateResurrectionGrid('multi'); const modalTitle = document.querySelector('#modal-role-selector h2'); if(modalTitle) modalTitle.style.display = 'none'; const modal = document.getElementById('modal-role-selector'); if(modal) modal.style.zIndex = "20000"; window.openModal('modal-role-selector'); };
window.openResurrectModal = function(playerId) { targetResurrectId = playerId; window.generateResurrectionGrid('single'); const modalTitle = document.querySelector('#modal-role-selector h2'); if(modalTitle) { modalTitle.style.display = 'block'; modalTitle.innerText = isDraftMode ? "CHANGER CARTE" : "RESSUSCITER"; } const modal = document.getElementById('modal-role-selector'); if(modal) modal.style.zIndex = "20000"; window.openModal('modal-role-selector'); };

window.openEventSelector = function(playerId, category) {
    targetResurrectId = playerId; targetEventCategory = category;
    const grid = document.getElementById('admin-role-grid'); if(!grid) return; grid.style.display = "block"; grid.innerHTML = "";
    const modalTitle = document.querySelector('#modal-role-selector h2'); if(modalTitle) { modalTitle.style.display = 'block'; modalTitle.innerText = `CARTE ${category.toUpperCase()}`; }
    const randomBtn = document.createElement('button'); randomBtn.className = "btn-validate btn-validate-small"; randomBtn.style.cssText = "margin-bottom:20px; background:linear-gradient(135deg, #2980b9, #3498db); border-color:#2980b9;"; randomBtn.innerText = "🎲 DONNER UNE ALÉATOIRE"; randomBtn.onclick = function() { window.adminDraw(playerId, category, true); }; grid.appendChild(randomBtn);
    const catGrid = document.createElement('div'); catGrid.className = "admin-grid-container"; 
    const cards = detectedEvents[category] || [];
    cards.forEach(imgSrc => {
        const div = document.createElement('div'); div.className = "role-select-item"; div.style.cssText = "cursor:pointer; text-align:center; padding:0;"; div.innerHTML = `<img src="${imgSrc}" loading="lazy" style="width:100%; height:auto; border-radius:6px; display:block;">`; div.onclick = function() { window.adminDraw(playerId, category, false, imgSrc); }; catGrid.appendChild(div);
    });
    grid.appendChild(catGrid);
    const modal = document.getElementById('modal-role-selector'); if(modal) modal.style.zIndex = "20000"; window.openModal('modal-role-selector');
};

window.adminDraw = function(playerId, category, isRandom, manualImg) {
    let finalCard = manualImg;
    if (isRandom) { const cards = detectedEvents[category]; if(cards && cards.length > 0) { finalCard = cards[Math.floor(Math.random() * cards.length)]; } }
    if (finalCard) { update(ref(db, `games/${currentGameCode}/players/${playerId}`), { drawnCard: { image: finalCard, category: category.toUpperCase() } }); window.closeModal('modal-role-selector'); internalShowNotification("Envoyé", `Carte ${category.toUpperCase()} donnée !`); } else { alert("Erreur : Aucune carte trouvée."); }
};

window.assignRoleToPlayer = function(roleId) {
    if(!targetResurrectId) return;
    if (isDraftMode) {
        update(ref(db, `games/${currentGameCode}/players/${targetResurrectId}`), { draftRole: roleId }).then(() => { window.closeModal('modal-role-selector'); distributionSelection.push(roleId); generateRoleChecklist(); });
    } else {
        if(confirm("Confirmer le changement de rôle ?")) {
            ...
            window.closeModal('modal-role-selector'); window.closeModal('modal-player-detail'); internalShowNotification("Succès", "Rôle modifié !");
        }
    }
};

function generateRoleChecklist() {
    const container = document.getElementById('roles-selection-list'); if(!container) return;
    const countSpan = document.getElementById('role-count'); if(countSpan) { countSpan.innerText = distributionSelection.length; }
    const rolesVillage = []; const rolesLoup = []; const rolesSolo = [];
    const groupedSelection = {}; distributionSelection.forEach(id => { groupedSelection[id] = (groupedSelection[id] || 0) + 1; });
    Object.entries(groupedSelection).forEach(([id, qty]) => {
        ...
        if(role) {
            ...
            if(role.category === 'village') rolesVillage.push(txt); else if(role.category === 'loups') rolesLoup.push(txt); else rolesSolo.push(txt);
        }
    });
    let summaryHTML = '';
    if (distributionSelection.length > 0) {
        summaryHTML = `<div class="summary-container"><div class="summary-col"><img src="Village.svg" alt="V"><strong>${rolesVillage.length}</strong>${rolesVillage.map(t => `<div class="summary-list-item">${t}</div>`).join('')}</div><div class="summary-col"><img src="Loup.svg" alt="L"><strong>${rolesLoup.length}</strong>${rolesLoup.map(t => `<div class="summary-list-item">${t}</div>`).join('')}</div><div class="summary-col"><img src="Solo.svg" alt="S"><strong>${rolesSolo.length}</strong>${rolesSolo.map(t => `<div class="summary-list-item">${t}</div>`).join('')}</div></div>`;
    }
    container.innerHTML = `${summaryHTML}<button class="btn-validate" onclick="window.openDistributionSelector()" style="background:#2c3e50; border-color:#34495e; margin-top:0;">📂 ${distributionSelection.length > 0 ? 'MODIFIER LA SÉLECTION' : 'CHOISIR LES RÔLES'}</button>`;
    get(child(ref(db), `games/${currentGameCode}/players`)).then((snapshot) => { const playerCount = snapshot.exists() ? Object.keys(snapshot.val()).length : 0; updateAdminButtons(playerCount); });
}

function distributeRoles() {
    let selectedRoles = [...distributionSelection];
    for (let i = selectedRoles.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [selectedRoles[i], selectedRoles[j]] = [selectedRoles[j], selectedRoles[i]]; }
    const playersRef = ref(db, 'games/' + currentGameCode + '/players');
    get(playersRef).then((snapshot) => {
        const players = snapshot.val(); if(!players) return;
        const playerIds = Object.keys(players);
        const updates = {};
        playerIds.forEach((id, index) => { if (selectedRoles[index]) { updates[`games/${currentGameCode}/players/${id}/draftRole`] = selectedRoles[index]; } });
        update(ref(db), updates);
    });
}

// --- FICHE TECHNIQUE JOUEUR (ADMIN) ---
window.openAdminPlayerDetail = function(playerId, playerPseudo, roleId, isDead, avatarBase64) {
    ...
    let campIcon = "Village.svg"; // Par défaut
    ...
    if(roleId && detectedRoles.length > 0) {
        ...
        if(r) { 
            roleImg = r.image; 
            roleTitle = r.title; 
            // Détection du camp pour l'icône SVG
            if(r.category === 'loups') campIcon = "Loup.svg";
            else if(r.category === 'solo') campIcon = "Solo.svg";
            else if(r.category === 'vampires') campIcon = "Vampires.svg";
        }
    }
    cardEl.src = roleImg;
    roleNameEl.innerText = roleTitle;

    // Injection de l'icône de camp
    if (document.getElementById('detail-camp-icon')) {
        document.getElementById('detail-camp-icon').src = campIcon;
    } else {
        // Si l'élément n'existe pas encore dans le HTML, on l'ajoute dynamiquement
        const icon = document.createElement('img');
        icon.id = 'detail-camp-icon';
        icon.src = campIcon;
        icon.className = 'camp-icon';
        roleNameEl.parentNode.insertBefore(icon, roleNameEl);
    }
    ...
    if(isDead) {
        const eventTypes = [ {id:'gold', label:'OR', col:'gold'}, {id:'silver', label:'ARGENT', col:'silver'}, {id:'bronze', label:'BRONZE', col:'#cd7f32'} ];
        const eventsContainer = document.createElement('div');
        eventsContainer.style.gridColumn = "1 / -1"; eventsContainer.style.display = "flex"; eventsContainer.style.gap = "5px"; eventsContainer.style.marginTop = "10px";
        eventTypes.forEach(t => {
            if(detectedEvents[t.id].length > 0) {
                const btn = document.createElement('button'); btn.className = "btn-admin-mini"; btn.style.flex = "1"; btn.style.padding = "10px"; btn.style.background = t.col; btn.style.color = "black"; btn.innerText = t.label;
                btn.onclick = function() { window.openEventSelector(playerId, t.id); }; eventsContainer.appendChild(btn);
            }
        });
...
function updateAdminUI(players) {
    ...
    if(isDraft) { distributionSelection = []; Object.values(players).forEach(p => { if(p.draftRole) distributionSelection.push(p.draftRole); }); generateRoleChecklist(); }
    ...
    if(count === 0) {
        ...
        Object.entries(players).forEach(([id, p]) => {
            ...
            if(!isDraft) {
                cardDiv.onclick = function() { window.openAdminPlayerDetail(id, p.name, currentRoleId, isDead, avatarSrc); };
            } else {
                const btnChange = document.createElement('button');
...
function updateAdminButtons(playerCount) {
    ...
    if (playerCount > 0 && selectedCount === playerCount) {
        btnDistribute.disabled = false; btnDistribute.style.background = "linear-gradient(135deg, #d4af37, #b8941f)"; btnDistribute.style.cursor = "pointer"; btnDistribute.innerHTML = isDraftMode ? "🃏 REMÉLANGER" : "🃏 PRÉPARER";
    } else {
        btnDistribute.disabled = true; btnDistribute.style.background = "grey"; btnDistribute.style.cursor = "not-allowed"; btnDistribute.innerHTML = `Attente (${playerCount}J / ${selectedCount}R)`;
    }
...
function revealRolesToEveryone() {
    ...
    get(playersRef).then((snapshot) => {
        ...
        const updates = {};
        Object.entries(players).forEach(([id, p]) => {
            if (p.draftRole) { updates[`games/${currentGameCode}/players/${id}/role`] = p.draftRole; updates[`games/${currentGameCode}/players/${id}/draftRole`] = null; updates[`games/${currentGameCode}/players/${id}/status`] = 'alive'; }
        });
...
function joinGame() {
    ...
    get(child(ref(db), `games/${code}`)).then((snapshot) => {
        if (snapshot.exists()) {
            ...
            localStorage.setItem('vm_player_code', currentGameCode); localStorage.setItem('vm_player_id', myPlayerId); localStorage.setItem('vm_player_name', pseudo);
            set(newPlayerRef, { name: pseudo, role: null, status: 'alive', avatar: avatar }).then(() => {
                document.getElementById('btn-join-action').style.display = 'none'; document.getElementById('player-lobby-status').style.display = 'block'; listenForPlayerUpdates();
            });
...
function revealRole(roleId) {
    ...
    if(roleData) {
        ...
        const panel = document.querySelector('.details-panel'); const overlay = document.querySelector('.details-overlay'); if(!panel || !overlay) return;
        panel.innerHTML = `<div id="online-content-wrapper" style="height:100%; display:flex; flex-direction:column; justify-content:center; align-items:center;"><button class="close-details" onclick="window.internalCloseDetails()" style="position:absolute; top:20px; right:20px; z-index:100; background:rgba(0,0,0,0.6); color:white; border:1px solid gold; border-radius:50%; width:40px; height:40px; font-size:20px;">✕</button><div class="scene-flip" onclick="this.classList.toggle('is-flipped')" style="margin:0;"><div class="card-object"><div class="card-face face-front"><img src="back.png" class="card-back-img" alt="Dos" style="width:100%; height:100%; object-fit:cover; border-radius:15px;"></div><div class="card-face face-back"><img src="${roleData.image}" alt="Rôle" style="width:100%; height:100%; object-fit:cover; border-radius:15px;"></div></div></div></div>`;
        panel.classList.add('active'); overlay.classList.add('active');
    }
}
...
function internalShowNotification(title, message) { const overlay = document.querySelector('.notification-overlay'); const box = document.querySelector('.notification-box'); if(overlay && box) { box.innerHTML = `<h3 style="color:gold;">${title}</h3><p>${message}</p><button onclick="document.querySelector('.notification-overlay').classList.remove('active')">OK</button>`; overlay.classList.add('active'); } else { alert(title + "\n" + message); } }
window.adminKill = function(playerId) { if(confirm("Confirmer la mort ?")) { update(ref(db, `games/${currentGameCode}/players/${playerId}`), { status: 'dead' }); } };
